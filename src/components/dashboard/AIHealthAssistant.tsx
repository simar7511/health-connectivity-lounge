  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      if (useFallbackMode) {
        ...
        // Call Hugging Face API via Firebase proxy
      ...
      if (error instanceof TypeError && error.message.includes("fetch")) {
        if (!apiError) setApiError("networkError");
        
        // Display network-specific toast
        toast({
          title: language === "en" ? "Network Error" : "Error de Red",
          description: language === "en" 
            ? "Unable to connect to the AI service. Check your internet connection." 
            : "No se puede conectar al servicio de IA. Compruebe su conexión a Internet.",
          variant: "destructive",
        });
        
        // Automatically switch to test mode after network error
        if (!useTestMode) {
          toggleTestMode(true);
          ...
          const errorMessage: Message = {
            id: Date.now().toString(),
            content: language === "en" 
              ? "I've switched to test mode due to network connectivity issues. You can still use simulated AI responses." 
              : "He cambiado al modo de prueba debido a problemas de conectividad de red. Aún puede usar respuestas de IA simuladas.",
            sender: "ai",
            timestamp: new Date(),
          };
